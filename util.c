...
void add_proc_list(char path[PATH_LEN], bool isPPS, bool aOption, bool uOption, bool xOption, unsigned long cpuTimeTable[PID_MAX])
{
	...
	//이전에 실행된 내역이 있을 경우 process 시작 시각 이후의 값이 아닌 이전 실행 이후의 값 불러옴
		if(cpuTimeTable[proc.pid] != 0){		
			update = true;
			//%cpu 갱신
			cpu = (totalTime-cpuTimeTable[proc.pid]) / (long double)(now - before) / hertz * 100;	
		}
...
void search_proc(bool isPPS, bool aOption, bool uOption, bool xOption, unsigned long cpuTimeTable[PID_MAX])
{
	...
	while((dentry = readdir(dirp)) != NULL){	// /proc 디렉터리 내 하위 파일들 탐색 시작
		...
		//PID 디렉터리인 경우 procList에 추가
		add_proc_list(path, isPPS, aOption, uOption, xOption, cpuTimeTable); 
	}
	closedir(dirp);
	return;
}
...
// 프로세스 출력 명령어를 실제 화면에 출력하는 함수
void print_pps(void){     

     // 열 출력
	//USER 출력
	if(uOption){printf("%-10s",USER_STR);}

	//PID 출력
	printf("%-8s",PID_STR);

	//%CPU 출력
	if(uOption){printf("%s\t",CPU_STR);}


	//%MEM 출력
	if(uOption){printf("%s\t",MEM_STR);}

	//VSZ 출력
	if(uOption){printf("%-10s",VSZ_STR);}

	//RSS 출력
	if(uOption){printf("%-10s",RSS_STR);}

	//TTY 출력
	printf("%-7s",TTY_STR);

	//STAT 출력
	if(aOption || uOption || xOption){printf("%-6s",STAT_STR);}

	//START 출력
	if(uOption){printf("%-8s",START_STR);}
	...
	if(aOption || uOption || xOption)
      ...
      // 프로세스 출력 시작
        for(int i = 0; i< procCnt; i++){
            //USER 출력
	    if(uOption){printf("%-10s",procList[i].user);}

	    //PID 출력
            printf("%-8lu",procList[i].pid);


	    //%CPU 출력
	    if(uOption){printf("%.1Lf\t",procList[i].cpu);}

	    //%MEM 출력
	    if(uOption){printf("%.1Lf\t",procList[i].mem);}

	    //VSZ 출력
	    if(uOption){printf("%-10lu",procList[i].vsz);}

	    //RSS 출력
	    if(uOption){printf("%-10lu",procList[i].rss);}

	    //TTY 출력
	    printf("%-7s",procList[i].tty);

	    //STAT 출력
	    if(aOption || uOption || xOption){printf("%-6s",procList[i].stat);}

	    //START 출력
	    if(uOption){printf("%-8s",procList[i].start);}

	     //TIME 출력
	     printf("%-7s",procList[i].time);

	     //COMMAND 또는 CMD 출력
	     if(aOption || uOption || xOption){
		    printf("%-.20s",procList[i].command);
	     }
	     else{
		    printf("%-.20s",procList[i].cmd);
	     }
